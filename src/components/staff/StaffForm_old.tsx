...
import { validateStaffData } from '../../services/staffsApi';
import styles from '../../styles/StaffsStyles/StaffForm.module.scss';
...
const StaffForm: React.FC<StaffFormProps> = ({
  ...
  const validateField = (field: keyof StaffFormData, value: string) => {
    const validation = validateStaffData({ ...formData, [field]: value });
      ...
      >
        <div className={styles.header}>
          <h2 id="staff-form-title" className={styles.title}>
            <i className={staff ? "fas fa-user-edit" : "fas fa-user-plus"} />
            {staff ? 'Chỉnh sửa khách hàng' : 'Thêm khách hàng mới'}
          </h2>
          <button
            type="button"
          ...
          <div className={styles.formActions}>
            ...
            >
              {isLoading ? (
                ...
                staff ? 'Cập nhật khách hàng' : 'Thêm khách hàng mới'
              )}
            </button>
          </div>
        </form>
