...
const Navbar: React.FC = () => {
  ...
  const buildNavLinks = (): NavLink[] => {
    ...
    // Only show Management dropdown if user is logged in
    if (isLoggedIn) {
      const links: NavLink[] = [...baseLinks];
      
      // Check if user is staff
      const isStaffRole = userProfile?.role?.toLowerCase().includes('staff') && 
                          !userProfile?.role?.toLowerCase().includes('evm');

      // Add top-level links for Manager only (NOT for staff)
      if (!isStaffRole) {
        links.push({ to: "/dealer-inventory", label: "Kho hàng", className: "inventory-link" });
        
        if (canCreateOrder()) {
          links.push({ to: "/delivery-status", label: "Đơn hàng", className: "orders-link" });
        }
        
        links.push({ to: "/staff", label: "Nhân viên", className: "staff-link" });
      } else {
        // Staff can only see Orders if they have permission
        if (canCreateOrder()) {
          links.push({ to: "/delivery-status", label: "Đơn hàng", className: "orders-link" });
        }
      }

      // Build dropdown items for "Quản lý" (remaining items)
      const dropdownItems: Array<{ to: string; label: string; icon: string }> = [
        ...
        { to: "/services-accessories", label: "Dịch vụ & Phụ kiện", icon: "fa-toolbox" }
      ];

      // Only add these items if user has full permission (manager/admin)
      // Note: Staff can still see Promotions and Services but in read-only mode
      // The pages themselves will handle the read-only logic based on canManagePromotions()
      ...
      links.push(managementLink);
      links.push({ to: "/compare-slots", label: "So sánh xe", className: "compare-link" });

      return links;
    }

    // When not logged in, only show basic links
    return [
      ...baseLinks,
            ...
            <>
              ...
              <div className="profile-dropdown">
                ...
                <div className="dropdown-menu">
                  ...
                  {/* Liên hệ - Moved to profile dropdown */}
